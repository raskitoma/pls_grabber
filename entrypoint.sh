...
# Set up the cron job
printf "%s env PATH=$PATH:/usr/bin:/opt env INFLUX_HOST_PORT=$INFLUX_HOST_PORT INFLUX_ORG=$INFLUX_ORG INFLUX_BUCKET=$INFLUX_BUCKET INFLUX_KEY=$INFLUX_KEY PLS_PRICE_URI=$PLS_PRICE_URI PLS_PRICE_API_KEY=$PLS_PRICE_API_KEY PLS_LAUNCH_URI=$PLS_LAUNCH_URI /usr/local/bin/python /app/plsgrabber.py > /proc/1/fd/1 2>&1\n%s env PATH=$PATH:/usr/bin:/opt env INFLUX_HOST_PORT=$INFLUX_HOST_PORT INFLUX_ORG=$INFLUX_ORG INFLUX_BUCKET=$INFLUX_BUCKET INFLUX_KEY=$INFLUX_KEY PLS_PRICE_URI=$PLS_PRICE_URI PLS_PRICE_API_KEY=$PLS_PRICE_API_KEY PLS_LAUNCH_URI=$PLS_LAUNCH_URI /usr/local/bin/python /app/pls_price_grabber.py > /proc/1/fd/1 2>&1\n" "$CRON_SCHEDULE" "$CRON_PRICE" | crontab -
